...
import static com.andrei1058.bedwars.BedWars.getSpongeParticleSupport;
...
public class SpongeAnimationTask extends BukkitRunnable {
    ...
    public void run() {
        if (radius > 4) {
            ...
            location.getWorld().playSound(location, Sound.SPLASH, 1, 1);
            return;
        }
        for (Location particle : getParticles(location, radius)) {
            getSpongeParticleSupport().play(particle);
        }
        location.getWorld().playSound(location, Sound.WOOD_CLICK, 1.0F, pitch/10F);
        radius++;
        pitch++;
    }
